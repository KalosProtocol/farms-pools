...
contract BnbStaking is Ownable {
    ...
    // The block number when XALO mining ends.
    ...
    constructor(
        ...
        // staking pool
        poolInfo.push(PoolInfo({lpToken: _lp, allocPoint: 1000, lastRewardBlock: startBlock, accXaloPerShare: 0}));
    ...
    function pendingReward(address _user) external view returns (uint256) {
        ...
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            ...
            uint256 xaloReward = multiplier.mul(rewardPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
